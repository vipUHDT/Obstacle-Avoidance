...
class WaypointMission:
    ...
    def __init__(self):
        ...
        self.search_area_waypoints = [
            (21.4002344, -157.7644624, 22),
            (21.4003056, -157.7642258, 30),
            (21.4004118, -157.7640527, 30),
            (21.4004817, -157.7638744, 30)
        ]

        self.payload_delivery_lat_long_comp   = [
        (21.4001171, -157.7644725, 1),
        (21.4004055, -157.7642325, 2),
        (21.4003618, -157.7642030, 3),
        (21.4001770, -157.7646589, 4)
        ...
        self.end_mission_latitude = 21.4002007
        self.end_mission_longitude = -157.7648252
    ...
    def reached_callback(self, msg):
        """
        ROS callback function to signal that waypoint has been reached
        """
    ...
    def attitude(self):
        ...
        # Get the latest altitude data (meters)
        ...
        rospy.loginfo(f"\nRoll: {self.roll}\nPitch: {self.pitch}\nYaw: {self.yaw}\nlat: {self.lat}\nlon: {self.lon}\naltitude: {self.alt}")
        rospy.loginfo(self.drone_sensory)
    ...
    def subprocess_execute(self, command):
        ...
        Record the execution time and success
    ...
    def deliver_payload_command(self):
        ...
        for i in range( len( self.payload_delivery_lat_long_comp ) ):

            print( f"Heading to payload #{i + 1}" )
            self.send_global_position_target(self.payload_delivery_lat_long_comp[0][i], self.payload_delivery_lat_long_comp[1][i], self.alt_AD)
            ...
            self.gpio_servo_command( self.payload_delivery_lat_long_comp[2][i], 0 )
            payloads_dropped += 1
...
if __name__ == '__main__':
    ...
    mission.set_param('WPNAV_RADIUS', 200) #cm
