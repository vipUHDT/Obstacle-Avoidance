...
class ObstacleDistance3D:
	...
	def __init__(self):
		...
		self.WAYPOINT_SPEED = 0.5 # m/s [OUTDATED]
		...
		#self.UAS_dk._parameters['OA_DB_EXPIRE'] = 10
		...
		self.UAS_dk._parameters['WPNAV_SPEED'] = 500 # cm/s
		self.UAS_dk._parameters['WPNAV_SPEED_DN'] = 500 #cm/s
		self.UAS_dk._parameters['WPNAV_SPEED_UP'] = 500 #cm/s
	...
	def send_waypoint(self, lat, lon, alt):
		...
		target_lat_int = int(lat * 1e7)
		target_lon_int = int(lon * 1e7)

		msg = self.UAS_dk.message_factory.set_position_target_global_int_encode(
			0,
			0,
			0,
	...
	def dk_waypoint_lap( self ):
		...
		for wp in range(len(self.waypoint_lap_latitude)):
			...
			#self.send_waypoint(self.waypoint_lap_latitude[ wp ], self.waypoint_lap_longitude[ wp ],self.waypoint_lap_alt[ wp ])
	...
	def pointcloud_callback(self, msg):
		...
		rospy.loginfo(f'Epoch Time: {ros_time}')
		ros_time_sec = ros_time.to_sec()
		ros_time_datetime = datetime.fromtimestamp(ros_time_sec)
		formatted_time = ros_time_datetime.strftime('%H:%M:%S')

		rospy.loginfo(f'Time: {formatted_time}')
